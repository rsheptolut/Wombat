...
BOOL TEXTURE_LOADER_BLP::Save(TEXTURE& Texture, CONST std::string& FileName, BUFFER& Buffer)
{
	...
	std::memcpy(&Buffer[CurrentOffset], &Header, sizeof(BLP_HEADER));
	CurrentOffset += sizeof(BLP_HEADER);

	std::memcpy(&Buffer[CurrentOffset], &JpegHeaderSize, sizeof(DWORD));
	CurrentOffset += sizeof(DWORD);

	Size = Header.Size[0] + JpegHeaderSize;
	std::memcpy(&Buffer[CurrentOffset], &((MipMapBufferList[0])[0]), Size);
	CurrentOffset += Size;
	...
	for(i = 1; i < NrOfMipMaps; i++)
	{
		...
		std::memcpy(&Buffer[CurrentOffset], &((MipMapBufferList[i])[JpegHeaderSize]), Header.Size[i]);
		CurrentOffset += Header.Size[i];
	}
