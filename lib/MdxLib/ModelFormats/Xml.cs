	...
	public sealed class CXml : IModelFormat
	{
		...
		public void Load(string Name, System.IO.Stream Stream, Model.CModel Model)
		{
			...
			using(System.Xml.XmlTextReader Reader = new System.Xml.XmlTextReader(Stream))
			{
				...
				System.Xml.XmlNode RootNode = Document.SelectSingleNode("model");
				if(RootNode == null) throw new System.Exception("Unable to load \"" + Name + "\", could not find the root node!");

				Xml.CLoader Loader = new Xml.CLoader(Name);
				Xml.CModel.Instance.Load(Loader, RootNode, Model);
		...
		public void Save(string Name, System.IO.Stream Stream, Model.CModel Model)
		{
			...
			using(System.Xml.XmlTextWriter Writer = new System.Xml.XmlTextWriter(Stream, CConstants.TextEncoding))
			{
				System.Xml.XmlDocument Document = new System.Xml.XmlDocument();

				Writer.Formatting = System.Xml.Formatting.Indented;
				Writer.WriteStartDocument();
				Writer.WriteComment(BuildHeader(Name));

				System.Xml.XmlNode RootNode = Document.CreateElement("model");
				Document.AppendChild(RootNode);

				Xml.CSaver Saver = new Xml.CSaver(Name);
				Xml.CModel.Instance.Save(Saver, RootNode, Model);
